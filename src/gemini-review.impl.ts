...
const reviewCode = async () => {
  try {
    ...
    const diff = await readFile("pr_diff.txt", "utf8");
    const changedFiles = await readFile("changed_files.txt", "utf8");
...
`;
    ...
    await writeFile("review_output.md", JSON.stringify(response));

    console.log("✅ Review generated successfully!");
    console.log(
      `📄 Review saved to review_output.md (${response.length} characters)`
    );
  } catch (_e) {
    // for typescript only
...
// Please check the workflow logs for more details.`;
    ...
    await writeFile("review_output.md", errorMessage);
    process.exit(1);
  }
};
